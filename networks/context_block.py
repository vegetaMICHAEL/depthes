...
class MultiAspectGCAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fusion_type == 'channel_mul':
            ...
            channel_out = channel_mul_term
        elif self.fusion_type == 'channel_add':
            # [N, C, 1, 1]
            channel_add_term = self.channel_add_conv(context)
            out = out + channel_add_term
            channel_out = channel_add_term
        else:
            # [N, C, 1, 1]
            channel_concat_term = self.channel_concat_conv(context)
            channel_out = channel_concat_term
        ...
        return out, channel_out

if __name__ == "__main__":
    from torchsummary import summary

    model = MultiAspectGCAttention(inplanes=256,ratio=0.0625,headers=4)
    summary(model, (256, 128, 256), device='cpu')