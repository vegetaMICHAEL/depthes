...
class Trainer:
    def __init__(self, settings):
        ...
        self.model = DeepLabv3_plus(pretrained=self.settings.pretrained, freeze_ASPP=self.settings.freeze_ASPP)
        print(self.model)
        ...
        if self.settings.load_weights_dir is not None:
            # self.load_model()
            self.model.load_state_dict(torch.load(os.path.join(self.settings.load_weights_dir, 'model.pth')), False)
        ...
        for batch_idx, inputs in enumerate(pbar):
            try:
                ...
                if early_phase or late_phase:
                    ...
                    pred_depth = outputs["pred"].detach()
    ...
    def process_batch(self, inputs):
        ...
        # outputs = self.model.encoder_decoder_forward(inputs)
        outputs = self.model.forward(inputs)
        losses['loss'] = self.compute_losses(inputs, outputs, self.settings.distribution)

        return outputs, losses

    def compute_losses(self, inputs, outputs, distribution):

        losses = self.compute_loss(inputs["gt_depth"],
                                   outputs["pred"],
                                   inputs["val_mask"])
        if outputs['q_z']:
            if distribution == 'normal':
                loss_KL = torch.distributions.kl.kl_divergence(outputs['q_z'], outputs['p_z']).sum(-1).mean()
            elif distribution == 'vmf':
                loss_KL = torch.distributions.kl.kl_divergence(outputs['q_z'], outputs['p_z']).mean()
            else:
                raise NotImplemented
            print('depth loss is {}, KL_loss is {}'.format(losses, loss_KL))
            #losses += loss_KL*0.0001
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.settings.batch_size)):  # write a maxmimum of four images
            ...
            writer.add_image("pred_depth/{}".format(j),
                             outputs["pred"][j].data / outputs["pred"][j].data.max(), self.step)
    ...
    def load_model(self):
        ...
        self.model.load_state_dict(torch.load(path), False)
